...
class Node {
    public String state;
    // AS "NEO Cordinates :Nx,Ny;NeoDamage:10
    // ;CarriedHostages:H1,H2;TotalAgents:A1:A1X:A1Y;"
    // +
    // "TotalHostages:H1:H1X:H1Y:H1D,H2:H2X:H2Y:H2D,H3:30,H4:100,H6:50,H5:10;Telephone
    // Booth Coordinates:Tx,Ty;"
    // +
...
class searchProblems {
    ...
    public int isGoalState(Node node) {
        ...
        if (node == null) { // not goal
            ...
            for (int i = 0; i < killed.length && !Killed.isEmpty(); i++) { // let calculate the number of the killed
                                                                           // hostages
            ...
            if ((SavedHostageArray.length + numberOfKilledHostages) == HostageArray.length && !Saved.isEmpty()
                    && !Hostages.isEmpty()) {// lets see if all hostages saved or killed
...
public class Matrix {
    ...
    public static String solve(String grid, String strategy, boolean visualize) {
        ...
        if (!OurGrid[6].isEmpty()) {
            ...
            for (int i = 0; i < Pads.length; i = i + 4) {
                pads = pads + "SP" + i + ":" + Pads[i] + ":" + Pads[i + 1] + "," + "FP" + i + ":" + Pads[i + 2] + ":"
                        + Pads[i + 3]
                        + ",";
            }
        ...
        n.state = OurGrid[2] + ";0;;" + agents + ";" + hostages + ";" + OurGrid[3] + ";" + ";" + ";" + pads + ";"
                + pills
                + ";" + CarryNumber;
    ...
    public static Queue<Node> stateSpace(Node n) {
        ...
        if (!n.operator.equals("fly")) {
            for (int i = 0; i < Pads.length && !NodeList[8].isEmpty(); i = i + 2) { // [sp1x,sp1y,fp1x,fp1y] pads
                ...
                if (((NeoX == sPx) && (NeoY == sPy)) || (((NeoX == ePx) && (NeoY == ePy)))) {// IS THERE A PAD HERE?
                    action = calculateMove("fly", n);
                    if (!checkstate(action)) { // IS IT ALREADY SAVED?
                        Children.add(action);// LETS ADD IT
                        addstate(action);
                    }
                }
        ...
        if (!n.operator.equals("drop") && !n.operator.equals("carry")) {
            for (int i = 0; i < Hostages.length && !NodeList[4].isEmpty(); i++) { // CARRY
                ...
                int Hx = Integer.parseInt(HostagesHelper[1]);
                int Hy = Integer.parseInt(HostagesHelper[2]);
                if ((NeoX == Hx && NeoY == Hy) && (!TB.equals(NeoC))// IS THERE HOSTAGE HERE?
        ...
        if (!n.operator.equals("drop") && !n.operator.equals("carry")) {
            if (NodeList[0].equals(NodeList[5])) { // DROP
                if (!NodeList[2].isEmpty()) { // IS CARRIED HOSTAGES LIST ISN'T EMPTY?
        ...
        if (!n.operator.equals("takePill")) {
            for (int i = 0; i < Pills.length && !NodeList[9].isEmpty(); i = i + 2) {// takePill
                ...
                if (NeoX == Px && NeoY == Py) { // IS THERE A PILL HERE?
        ...
        if (NeoX > 0 && (!n.operator.equals("down"))) { // UP [A0:x:y],[A1:x:y]
            for (int i = 0; i < Hostages.length && !NodeList[4].isEmpty(); i++) { // CHECK IF THERE IS A HOSTAGE WITH 98
                                                                                  // HP HERE?
                String[] HostagesHelper = Hostages[i].split(":");// [[H1,H1X,H1Y,H1D],,,,]
                int Hx = Integer.parseInt(HostagesHelper[1]);
                int Hy = Integer.parseInt(HostagesHelper[2]);
            ...
            for (int i = 0; i < Agents.length && !NodeList[3].isEmpty(); i++) { // CHECKS IF THERE IS AN AGENT HERE?
                String[] agentHelper = Agents[i].split(":");// [A0],[X],[Y]
                if (!Agents[i].isEmpty()) {
                    if (((NeoX - 1) == Integer.parseInt(agentHelper[1]))
                            && (NeoY == Integer.parseInt(agentHelper[2]))) {
                        isagenthere = true;
            ...
            if (!isagenthere && !agentkilled && !ishostagehere) {// THERE IS NOT AGENT THERE OR HOSTAGE WITH 98 DAMAGE
    ...
    public static Node calculateMove(String action, Node parent) {
        ...
        switch (action) {
            ...
            case "carry":
                ...
                for (int i = 0; i < Hostages.length && !NodeList[4].isEmpty(); i++) {
                    String[] HostagesHelper = Hostages[i].split(":");
                    if (((NeoX == Integer.parseInt(HostagesHelper[1])) && (NeoY == Integer.parseInt(HostagesHelper[2])))
                            && !NewCarriedHostages.contains(HostagesHelper[0])) {
            ...
            case "fly":
                ...
                break;
            case "kill":
                if (!NodeList[3].isEmpty()) {// IF THERE IS ANY NEIGHBOUR AGENT KILL HIM
                    TotalAgents = "";
                    for (int i = 0; i < Agents.length && !NodeList[3].isEmpty(); i++) { // [A0...,A1...]
                        ...
                        if (!Agents[i].isEmpty() && !NodeList[7].contains(agentHelper[0])) {
                            ...
                            if ((NeoX == Integer.parseInt(agentHelper[1]))
                                    && ((NeoY - 1) == Integer.parseInt(agentHelper[2]))) {
                                if (Killed.length() != 0)
                                    Killed += ",";
                                Killed += agentHelper[0];
                                child.operator = "kill";
                ...
                if (child.operator.equals("kill")) { // UPDATE THE DAMAGE
                    NeoDamage += 20;
                }
                break;
            default:
        ...
        // DOWN LEFT RIGHT , ALSO TOTAL AGENTS TO DECLARE IF SOMEONE DIED
        for (int i = 0; i < Hostages.length && !NodeList[4].isEmpty(); i++) { // EveryTick
            ...
            for (int J = 0; J < Agents.length && !NodeList[3].isEmpty(); J++) { // if he is already agent or he will be
                                                                                // killed
                String[] agentHelper = Agents[J].split(":");
                if (!Agents[J].isEmpty()) {// WILL HE BE KILLED?
                    if ((((NeoX - 1) == Integer.parseInt(agentHelper[1])) && (NeoY == Integer.parseInt(agentHelper[2])))
                            || (((NeoX + 1) == Integer.parseInt(agentHelper[1]))
                                    && (NeoY == Integer.parseInt(agentHelper[2])))
                            || ((NeoX == Integer.parseInt(agentHelper[1]))
                                    && ((NeoY - 1) == Integer.parseInt(agentHelper[2])))
                            || ((NeoX == Integer.parseInt(agentHelper[1]))
                                    && ((NeoY + 1) == Integer.parseInt(agentHelper[2])))) {
                        isKilled = true;
            ...
            if ((NeoX == Hx && NeoY == Hy) && (!TB.equals(NeoC))) {
                if ((CarryNumber > CarriedHostages.length || (NodeList[2].isEmpty() && CarryNumber > 0)) && !wasagent
                        && !wasKilled && !wasCarried) {
                    isCarried = true;
                }
            }
            ...
            if (!wasSaved && !isSaved && actiondone) {
                HostagesHelper[3] = String.valueOf(Integer.parseInt(HostagesHelper[3]) + 2);// UPDATE HOSTAGES DAMAGE
            }
            if ((Integer.parseInt(HostagesHelper[3]) >= 100)) {
                HostagesHelper[3] = "100";
                if (!wasCarried && !wasagent && !wasKilled && !wasSaved && !isCarried && !isKilled && !isSaved) {// update
                                                                                                                 // TotalAgents
                    if (TotalAgents.length() != 0) {
                        TotalAgents += ",";
                    }
    ...
    public static String NodePaths(Node n, int nodes) {
        // IT GOES FROM GOAL NODE TO THE INITIAL TO SAVE THE PATH FROM THE OPERATORS
    ...
    // Method that saves expanded nodes depending on the type of search and iterates
    // from initial to goal, stating whether its complete or not
